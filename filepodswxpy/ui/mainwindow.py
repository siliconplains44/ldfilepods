...
class MainWindow(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        if (True == versionInformation["newversionavailable"]):
            ...
            self.bankfileDownloaders = []
            self.bankfileUploaders = []

            for i in range(4):
                newfileDownloader = filedownloader.FileDownloader()
                newfileDownloader.id = i
                newfileDownloader.Start()
                self.bankfileDownloaders.append(newfileDownloader)

            for i in range(12):
                newfileUploader = fileuploader.FileUploader()
                newfileUploader.id = i
                newfileUploader.Start()
                self.bankfileUploaders.append(newfileUploader)
    ...
    def ShutdownApplication(self):
        ...
        for fileUploader in self.bankfileUploaders:
            fileUploader.Stop()

        for fileDownloader in self.bankfileDownloaders:
            fileDownloader.Stop()
