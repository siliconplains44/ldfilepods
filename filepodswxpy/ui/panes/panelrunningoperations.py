...
class PanelRunningOperations(wx.Panel):
    ...
    def UpdateGrid(self):
        ...
        for index, runningOperation in enumerate(self.runningOperations):
            ...
            if (runningOperation.IsRunning == True):
                status = 'Running'
            else:
                status = 'Stopped'
            ...
            self.m_gridData.SetCellValue(index, 3, runningOperation.Started)
            self.m_gridData.SetCellValue(index, 4, runningOperation.Ended)
            self.m_gridData.SetCellValue(index, 5, str(runningOperation.totalCountFiles))
            self.m_gridData.SetCellValue(index, 6, str(runningOperation.totalCountFolders))
            self.m_gridData.SetCellValue(index, 7, str(runningOperation.totalSizeInBytes))
            self.m_gridData.SetCellValue(index, 8, str(runningOperation.totalFilesProcessed))
            self.m_gridData.SetCellValue(index, 9, str(runningOperation.totalFoldersProcessed))
            self.m_gridData.SetCellValue(index, 10, str(runningOperation.totalBytesProcessed))

            percentComplete = "0"

            if runningOperation.totalSizeInBytes != 0:
                prevalue = Decimal(
                    Decimal(runningOperation.totalBytesProcessed) / Decimal(runningOperation.totalSizeInBytes)) * 100
            ...
            if (runningOperation.CompletedSuccessfully == True):
