...
dataAccessLayerX.prototype.getConnectionExtended = function(ConnectionString)  {
    var self = this;

    var splitConnString = ConnectionString.split(';');

    var databasehost = splitConnString[0].split('=')[1];
    var databaseport = splitConnString[1].split('=')[1];
    var databaseusername = splitConnString[2].split('=')[1];
    var databasepassword = splitConnString[3].split('=')[1];
    var database = splitConnString[4].split('=')[1];
    var ssl = fs.readFileSync(config.cert);
...
dataAccessLayerX.prototype.openConnectionExtended = function(ConnectionString, cb) {
    var self = this;
    self.getConnectionExtended(ConnectionString);
    self.connection.connect(null, cb);
}
